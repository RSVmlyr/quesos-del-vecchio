...
<section class="relative h-screen w-screen bg-beige-3 flex items-center">
    <div class="absolute container z-10 top-[9.5625rem] inset-x-0 lg:top-[12.8125rem]">
        <div class="max-w-80 grid gap-2 lg:max-w-[22.9375rem] lg:gap-4">
    ...
    <?php if ($children && $children->have_posts()) : ?>
        ...
        <div class="absolute container z-10 bottom-6 inset-x-0 grid gap-12 lg:grid-cols-2 lg:items-end lg:bottom-12">
            <ul class="grid gap-4">
                <?php while ($children->have_posts()) : 
                    $children->the_post(); 
                ...
                ?>
                    <li>
                        <a class="text-white font-gazpacho text-[2.5rem] font-medium leading-none flex justify-between items-center tracking-tight lg:text-8xl" href="<?php the_permalink(); ?>">
                            <?php echo $shortname; ?>

                            <div class="bg-beige-3 rounded-full p-2 text-blue w-10 h-10 flex items-center justify-center lg:hidden">
